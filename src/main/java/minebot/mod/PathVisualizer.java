 ...
 * would be by default.
 *
 * Re-added after the state-machine architecture rewrite (see
 * STATE_MACHINE.md) -- each node now owns its own PathTracker instance
 * (LegsNavigateNode's, currently the only one) rather than one shared
 * ControlState.pathTracker every mode aliased, so this takes whichever
 * PathTracker instance is passed in rather than assuming there's exactly
 * one canonical instance to reach via ControlState.
 */
